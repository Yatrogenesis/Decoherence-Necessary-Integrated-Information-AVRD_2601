...
Crucially, IIT does not define $\Phi$ as a generic information-theoretic quantity, but as a measure of \textit{intrinsic causal irreducibility} grounded in classical probability distributions over system states. This ontological commitment---that consciousness corresponds to irreducible cause-effect power rather than quantum coherence per se---is foundational to interpreting our results.
...
While the Schmidt basis is used here for analytical clarity, the vanishing of $\Phi$ for pure states is not a basis-dependent artifact. Rather, it reflects IIT's requirement that integrated information arise from \textit{classical} statistical dependencies in the probability distribution over states. Pure quantum states, regardless of basis choice, do not generate such dependencies because all correlations are encoded in deterministic phase relationships rather than statistical mixtures.

Mixed states, by contrast, can exhibit genuine statistical correlations that resist such factorization.

The result $\Phi = 0$ for pure quantum states is not contingent on system size, coupling strength, or entanglement structure---it is a \textit{logical consequence} of how IIT defines integrated information. Any extension of IIT that yields $\Phi > 0$ for pure states would require redefining the measure itself, not merely improving computational methods.
...
\subsection{Scope and Methodological Notes}

The exclusion of off-diagonal density matrix elements is not a technical limitation but a direct consequence of IIT's causal-probabilistic framework, which defines $\Phi$ over classical state distributions. A ``fully quantum'' definition of integrated information would constitute a different theory, not a refinement of IIT. Our results are therefore exact within IIT's ontological commitments, not approximations awaiting correction.

Regarding system size: while our quantum reservoirs are small compared to biological neural networks, the mathematical result $\Phi = 0$ for pure states holds independently of scale. Larger systems would exhibit the same qualitative behavior---the necessity of decoherence for $\Phi > 0$ is a structural feature, not a finite-size artifact.
...
These findings suggest that consciousness, understood as integrated information, is fundamentally tied to thermodynamic irreversibility. A conscious system must be open to its environment, subject to noise, and operating in the classical-quantum boundary regime.

Our results do not rule out quantum effects in physical substrates supporting consciousness; rather, they demonstrate that such effects must operate \textit{through} decoherence processes that generate classical statistical structure. The ``quantum consciousness'' hypothesis, in its strongest form positing that quantum coherence directly generates or enhances consciousness, appears inconsistent with Integrated Information Theory as currently formulated. However, quantum dynamics may play essential roles in \textit{producing} the mixed states that exhibit $\Phi > 0$---the necessity is thermodynamic openness, not the absence of quantum mechanics.
